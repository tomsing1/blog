...
webr: 
  ...
  packages: ['ggvenn', 'huxtable', 'readr']
filters:
  - webr
editor_options: 
...
results <- lapply(kResults, \(result) {
  ...
  read_csv(url)[, c("gene_name", "adj.P.Val", "logFC")]
})
names(results) <- names(kResults)

# keep only genes present in all results
keep <- Reduce(intersect, sapply(results, "[[", "gene_name"))
results <- lapply(results, \(result) result[result$symbol %in% keep, ])
...
_separately_ for each batch/day.[^1]
...
```{webr-r}
#| context: interactive
results[["Day1"]][match(selected, results[["Day1"]]$symbol), ] |>
  huxtable::hux()
```
...
```{webr-r}
#| context: interactive
results[["Day2"]][match(selected, results[["Day2"]]$symbol), ] |>
  huxtable::hux()
```
...
```{webr-r}
#| context: interactive
results[["Day3"]][match(selected, results[["Day3"]]$symbol), ] |>
  huxtable::hux()
```
