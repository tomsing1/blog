...
The updated data.frame requires `r format(object.size(df2), units = "MB")`, just
...
```{r}
o <- as(counts(tenx_pbmc4k), "dgCMatrix")
dim(o)
colnames(o) <- tenx_pbmc4k$Barcode
format(object.size(o), unit = "MB")
```

```{r}
n <- as(da, "dgCMatrix")
dim(n)
format(object.size(n), unit = "MB")
```
