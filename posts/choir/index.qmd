...
1. Using multiple cores speeds up the computation.

Alternatively, we can perform the exact same calculations in a stepwise fashion:
...
and to plot the results of the sub-clustering analysis:
...
The example dataset does not contain `batch` information - so we just
just assign cells to two random batches (`A` and `B`) to demonstrate this
functionality
...
we just create a random indicator variable for demonstration purposes.
