...
on how to _create_ and _populate_ a new database from scratch. Here, I explore
both noSQL and SQL approaches to data management.
...
in the referenced table, and the following SQL
expression yields `0`:

```{r}
dbGetQuery(con, "PRAGMA foreign_keys")[["foreign_keys"]]
```

To enable foreign key checking, we use the following command:

```{r}
dbExecute(con, 'PRAGMA foreign_keys=ON;')
```

Now the same query we issued above confirms that the foreign key check is
active.
...
For each of the tables we want to create, we first check whether
a table with the same name already exists, and delete it if necessary. 

The we use the `dbAppendTable` function, and not the `dbWriteTable`
command, because `dbWriteTable` will overwrite an existing table and all
of the constraints we have enforced in the `CREATE TABLE` statment (e.g. primary 
key, foreign key relationships, field types, etc) will be lost.

```{r results='hide'}
# delete existing tables, if any
for (table_name in c("tbl_geneset", "tbl_element", "tbl_elementset")) {
  if (dbExistsTable(con, table_name)) {
    dbExecute(con, paste0("DROP TABLE ", table_name))
  }
}
# create and populate the three tables
dbExecute(con, 
          "CREATE TABLE tbl_geneset (geneset TEXT PRIMARY KEY, url TEXT)")
dbAppendTable(con, name = "tbl_geneset", value = geneset)

dbExecute(con, 
          "CREATE TABLE tbl_element (element TEXT PRIMARY KEY, entrezid TEXT)")
dbAppendTable(con, name = "tbl_element", value = element)
...
dbAppendTable(con, name = "tbl_elementset", value = elementset)
```

We now have three tables in the database:
...
Let's take a peek at the foreign keys we defined in the `tbl_elementset` table,
to make sure they are correct.

```{r}
dbGetQuery(con, "PRAGMA foreign_key_list('tbl_elementset')")
```
There are two foreign keys, one connecting to the `element` field in the 
`tbl_element` table, and the other connecting to the `geneset` field in the
`tbl_geneset` table. Looks good!
