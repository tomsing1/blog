...
knock-out animals were included in the study, so we exclude the male animals
...
All cell types are represented in animals from both genotypes:
...
As expected, most microglial states described in the paper are represented in 
both genotypes, albeit some of the states are more frequently observed in
one than the other:
...
#| message: false
#| warning: false
fit <- lemur(sce[, selected_cells], design = ~ genotype,
