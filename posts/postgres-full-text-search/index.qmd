...
We have successfully indexed and searched the existing three entries in our
database. But what if we add new information? Let's create a new record for the
`euro` dataset.
...
To enter this record, we not only have to populate the `name`, `title` and
...
Now, when we search for the term `data`, we find both the original and the
new record:
